package org.diableAvionics.weapons;
...
public class DerechoEffect implements EveryFrameWeaponEffectPlugin {
        ...
        if (theSystem.isOn()){
            ...
            if (timer.intervalElapsed()){
                ...
                for (MissileAPI m : missiles){
                    ...
                    if(!locked.contains(m) && !ModPlugin.DERECHO_IMMUNE.contains(m.getProjectileSpecId())){
                        locked.add(m);
                        if(!ModPlugin.DERECHO_RESIST.contains(m.getProjectileSpecId())){
                            vulnerable.add(m);
                        }
                    }
                }
